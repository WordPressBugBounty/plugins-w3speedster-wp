...
class w3speedster_css extends w3speedster{
	...
	function w3ConvertArrRelativeToAbsolute($string, $url, $matches){
		...
		foreach($matches as $match){
			...
			if(substr($match1, 0, 1) == '/' || strpos($match1,'http') !== false){
				$match1 = is_file($this->add_settings['wp_document_root'].'/'.trim($match_arr['path'],'/')) ? $this->add_settings['wp_site_url'].'/'.trim($match_arr['path'],'/') : $match1;
				$import_match = $match1;
			}else{
			...
			if(strpos($match1,'.css')!== false && strpos($string,'@import '.$match)!== false && $url != $this->add_settings['wp_home_url'].'/index.php'){
                $string = str_replace('@import '.$match.';',$this->w3RelativeToAbsolutePath($this->removeDotPathSegments($import_match),$this->w3speedsterGetContents($this->removeDotPathSegments(str_replace($this->add_settings['wp_site_url'],$this->add_settings['wp_document_root'],$import_match)))), $string);
                continue;
			}
			...
			if(!empty($this->settings['webp_png']) && in_array($ext, $webp_enable) && !empty($imgsrc_webpfilepath)){
				if(is_file($imgsrc_webpfilepath) && (!empty($this->add_settings['disable_htaccess_webp']) || !is_file($this->add_settings['wp_document_root']."/.htaccess") || $this->add_settings['image_home_url'] != $this->add_settings['wp_site_url'] )){
					...
					if(!empty($this->settings['opt_img_on_the_go'])){
						$response = $this->w3OptimizeAttachmentUrl(str_replace($this->add_settings['wp_site_url'],$this->add_settings['wp_document_root'],$img_arr[0]));
					}
				}
			}
	...
	function w3CreateFileCacheCss($path){
        ...
        if( !file_exists($cache_file_path) ){
			$css = $this->w3speedsterGetContents($this->add_settings['wp_document_root'].$path);
    ...
    function minifyCss($css_links){ 
		...
		if(!empty($css_links) && !empty($this->settings['css'])){
			...
			foreach($css_links as $key => $css){
				$css_obj = $this->w3ParseLink('link',str_replace($this->add_settings['image_home_url'],$this->add_settings['wp_site_url'],$css));
			...
			foreach($css_links_arr as $key => $link_arr){
				...
				if(!empty($css_obj['rel']) && $css_obj['rel'] == 'stylesheet' && !empty($css_obj['href'])){
					...
					if(!$this->w3IsExternal($css_obj['href'])){
						if($this->w3Endswith($css_obj['href'], '.php') || strpos($css_obj['href'], '.php?') !== false ){
						...
						}elseif(!is_file($this->add_settings['wp_document_root'].$url_array['path'])){
							...
							$url_array['path'] = $this->w3CreateFileCacheCssUrl($css_obj['href']);
							$css_obj['href'] = $this->add_settings['home_url'].$url_array['path'];*/
						}elseif(filesize($this->add_settings['wp_document_root'].$url_array['path']) > 0){
							$org_css = $url_array['path'];
							$url_array['path'] = $this->w3CreateFileCacheCss($url_array['path']);
							$css_obj['href'] = $url_array['path'];
						}else{
							if($this->w3Endswith($css_obj['href'], '.php') || strpos($css_obj['href'], '.php?') !== false || filesize($this->add_settings['wp_document_root'].$url_array['path']) < 1 ){
								$this->w3StrReplaceSetCss($css,'');
							}
							continue;
						}
					...
					if(!empty($src) && !$this->w3IsExternal($src) && $this->w3Endswith($src, '.css')){
						$filename = $this->add_settings['wp_document_root'].$url_array['path'];
			...
			$this->w3InsertContentHead('<style id="w3_bg_load">div, section, iframelazy, iframe{background-image:none !important;}</style><style>div[data-w3BgLazy="1"],section[data-w3BgLazy="1"],iframelazy[data-w3BgLazy="1"], iframe[data-w3BgLazy="1"]{background-image:none !important;}</style><style id="w3speedster-custom-css">'.$all_inline_css.'</style>',4);
	...
	function w3GetCssUrl($css,$enable_cdn){
		if($enable_cdn){
			$css = str_replace($this->add_settings['wp_site_url'],$this->add_settings['image_home_url'],(strpos($css,$this->add_settings['wp_site_url']) === false ? $this->add_settings['wp_site_url'] : '').$css);
		}else{
