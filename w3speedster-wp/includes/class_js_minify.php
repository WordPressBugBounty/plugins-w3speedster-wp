...
class w3speedster_js extends w3speedster_css{
	...
	function w3CreateFileCacheJsUrl($path){
		...
		if(!file_exists($cache_file_path)){
			...
			return str_replace($this->add_settings['wp_document_root'],'',$cache_file_path);
		}
	}
	function w3_changes_in_js($string,$path=''){
		...
		if(strpos($string,'DOMContentLoaded') === false){
			$string = preg_replace('/(jQuery|\$)\(\s?function\s?\(\s?((?!jQuery)[\S|\$]{1,6})\s?\)\s?{/', 'jQuery(document).on(\'w3-DOMContentLoaded\',function(){const $2=jQuery;', $string);
			$string = preg_replace('/(jQuery|\$)\(\s?function\s?\(\s?(jQuery|[\s]{0,4})\s?\)\s?{/', 'jQuery(document).on(\'w3-DOMContentLoaded\',function(){', $string);
			$string = preg_replace('/(jQuery|\$)(\s?\(\s?)([\(\s]?\(\s?\(\s?\)=>)/', 'document.addEventListener(\'w3-DOMContentLoaded\',$3', $string);
			$string = preg_replace('/\s{1}(\S{1,6})\(document\)\s?\.on\s?\([\'|\"]\s?ready\s?[\'|\"]\s?,(\s?function\s?\(\s?((?!jQuery)[\S|\$]{1,6})\s?\)\s?{)/', ' jQuery(document).on(\'w3-DOMContentLoaded\',function(){const $3=jQuery;', $string);
			$string = preg_replace('/(jQuery|\$)(\s?\(\s?document\)\s?)\.ready\(function\s?\((\s?(?!jQuery)[\S|\$]{1,6})\s?\)\s?{/', 'jQuery(document).on(\'w3-DOMContentLoaded\',function(){const $3=jQuery;', $string);
			$string = preg_replace('/(jQuery|\$)(\s?\(\s?document\)\s?)\.ready\(function\s?\((\s?(jQuery|[\s]{0,4}))\s?\)\s?{/', 'jQuery(document).on(\'w3-DOMContentLoaded\',function(){', $string);
		}
		$string = preg_replace('/(document\)\s*\.on\s*\([\'|\"]\s*)load(\s*[\'|\"]\s*,\s*\s*function\s*\(\s*\S*\s*\))/', "$1w3-load$2", $string);
		$string = preg_replace('/(window\)\s*\.on\s*\([\'|\"]\s*)load(\s*[\'|\"]\s*,\s*\s*function\s*\(\s*\S*\s*\))/', "$1w3-load$2", $string);
    ...
    function w3CreateFileCacheJs($path){
        ...
        if( !file_exists($cache_file_path) ){
			...
			array_pop($path1);
			$path1 = implode('/',$path1);
            $string = $this->w3speedsterGetContents($this->add_settings['wp_document_root'].$path);
            $string = $this->w3ModifyFileCacheJs($string, $path);
            $this->w3CreateFile($cache_file_path, $string );
        }
	    return str_replace($this->add_settings['wp_document_root'],'',$cache_file_path);
    ...
    function minify($script_links){
		...
		if(!empty($script_links) && !empty($this->settings['js'])){
			...
			for($si=0; $si < count($script_links); $si++){
				...
				$script_obj = !empty($this->add_settings['script_obj'][$si]) ? $this->add_settings['script_obj'][$si] : $this->w3ParseLink('script',str_replace($this->add_settings['image_home_url'],$this->add_settings['wp_site_url'],$script_links[$si]));
				$script_text = '';
                ...
                if(!empty($script_obj['src'])){
					...
					if(!$this->w3IsExternal($script_obj['src']) && $this->w3Endswith($url_array['path'], '.js') && $exclude_js != 3){
                        ...
                        if(file_exists($this->add_settings['wp_document_root'].$url_array['path'])){
					...
					if($exclude_js){
						...
						if(file_exists($this->add_settings['wp_document_root'].$url_array['path']) && strpos( $this->w3speedsterGetContents($this->add_settings['wp_document_root'].$url_array['path']),'jQuery requires a window with a document') !== false){
							$this->add_settings['jquery_excluded'] = $this->add_settings['wp_document_root'].$url_array['path'];
						}
						$script_obj['src'] = $enable_cdn && $enable_cdn_path ? str_replace($this->add_settings['wp_site_url'],$this->add_settings['image_home_url'] ,$script_obj['src']) : $script_obj['src'];
		...
		if(!empty($final_merge_has_js)){
			...
			for($ii = 0; $ii < count($final_merge_has_js); $ii++){
				if($ii == count($final_merge_has_js) -1 ){
					$this->w3StrReplaceSet($final_merge_has_js[$ii],'<script type="lazyJs" data-src="'.$cache_js_url.'"></script>');
				}else{
		...
		if(!empty($file_name)){
			...
			if(!is_file($this->w3GetCachePath('all-js').'/'.$js_file_name)){
				...
				foreach($final_merge_js as $key => $script_path){
					$all_js .= $this->w3speedsterGetContents($this->add_settings['wp_document_root'].$script_path).";\n";
				}
    ...
    function w3LazyLoadJavascript(){
        ...
        $script = 'var w3elem;setTimeout(function(){w3elem = window.innerWidth<768?\'touchstart\':\'click\';});var w3_lazy_load_by_px='.$lazy_load_by_px.', blank_image_webp_url = "'. (($enable_cdn && !in_array('.png',$exclude_cdn_arr)) ? str_replace($this->add_settings['wp_site_url'],$this->add_settings['image_home_url'],$this->add_settings['upload_base_url']): $this->add_settings['upload_base_url']).'/blank.pngw3.webp", w3_upload_path="'.$this->add_settings['upload_path'].'", w3_webp_path="'.$this->add_settings['webp_path'].'", w3_mousemoveloadimg = false, w3_page_is_scrolled = false, w3_lazy_load_js = '.(!empty($this->settings['load_combined_js']) && $this->settings['load_combined_js'] == 'after_page_load' ? 1 : 0).', w3_js_is_excluded = '.(!empty($this->settings['js_is_excluded']) ? 1 : 0).', w3Inp = '.(!empty($this->settings['enable_inp']) ? 1 : 0).',';
		if((!empty($this->settings['exclude_page_from_load_combined_js']) && $this->w3CheckIfPageExcluded($this->settings['exclude_page_from_load_combined_js'])) || empty($this->settings['js']) ){
			$script.='w3_excluded_js=1;';
        }else{
			$script.='w3_excluded_js=0;'; 
		}
		return $script.$this->w3speedsterGetContents(W3SPEEDSTER_PLUGIN_DIR.'assets/js/script-load.min.js');
	}
	function w3LazyLoadImages(){
		$inner_script_optimizer = $this->w3speedsterGetContents(W3SPEEDSTER_PLUGIN_DIR.'assets/js/img-lazyload.js');
	...
	function loadScriptTagInUrl($script_tag,$i){
		...
		$file_name = get_option('w3_rand_key').$i;
