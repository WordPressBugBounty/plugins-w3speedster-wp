(function() {
    ...
    let allEventsAfter = [], allEventsBefore = [], triggerEvents = [];
	triggerEvents['click'] = [];
	window.w3Html = [],window.w3Html['class']='';
	EventTarget.prototype.addEventListener = function(eventName, eventHandler,eventOptions) {
        if(['click','touchstart','keydown','keyup','input','blur','focus','pointerup','pointerdown'].includes(eventName)){
			if(window.w3Html['class'].indexOf('w3_start') != -1){
				allEventsAfter[eventName] = typeof(allEventsAfter[eventName]) == 'object' ? allEventsAfter[eventName] : [];
				allEventsAfter[eventName].push(this);
			}else{
				allEventsBefore[eventName] = typeof(allEventsBefore[eventName]) == 'object' ? allEventsBefore[eventName] : [];
				allEventsBefore[eventName].push(this);
			}
			/*if(w3Inp && ['keydown','keyup','input','blur','focus','pointerup','pointerdown'].includes(eventName) && eventHandler.toString().indexOf('preventDefault') == -1 && eventHandler.toString().indexOf('stopPropagation(') == -1 && eventHandler.toString().indexOf('event.dispatch.apply') == -1){
				if((typeof(eventOptions) != 'object') || (typeof(eventOptions) == 'object' && typeof(eventOptions['passive']) != 'boolean')){
					eventOptions = 	typeof(eventOptions) == 'object' ? eventOptions : [];
					eventOptions['passive'] = !0;
				}
			}*/
		}
		if(w3Inp && window.location.href.indexOf('?tester=') != -1){
			console.log("rocket",this,eventName,eventHandler,eventOptions);
		}
		oldAddEventListener.call(this, eventName, eventHandler,eventOptions);
	};
	function w3BodyClick(e){
		if(e.target.tagName == 'I'){
			triggerEvents['click'].push(e.target.parentNode);
		}else{
			triggerEvents['click'].push(e.target);
		}
	}
    document.querySelector('html').addEventListener('click',w3BodyClick,{passive:true});
	window.addEventListener('w3-load',function(){
		document.querySelector('html').removeEventListener('click', w3BodyClick);
		setTimeout(function(){
			triggerListener('click');
		},100);
	});
	function triggerListener(k){
		triggerEvents[k].forEach(function(e){
			allEventsAfter[k].forEach(function(o){
				if(e == o){
					var exists = 0;
					allEventsBefore[k].forEach(function(i){
						if(i == o){
							exists = 1;
						}
					});
...
class w3_loadscripts {
	...
	async load_style_resources(){
		...
		this.style_load_fired=!0,this.register_styles(),document.getElementsByTagName('html')[0].setAttribute('data-css',this.w3_styles.length),document.getElementsByTagName('html')[0].setAttribute('data-css-loaded',0),this.preload_scripts(this.w3_styles), this.load_styles_preloaded(),this.load_fonts(this.w3_fonts)
	}
	...
	register_scripts() {
		document.querySelectorAll("script[type=lazyJs]").forEach((e => {
			e.hasAttribute("data-src") ? e.hasAttribute("async") && !1 !== e.async ? this.w3_scripts.async.push(e) : e.hasAttribute("defer") && !1 !== e.defer || "module" === e.getAttribute("data-w3-type") ? this.w3_scripts.defer.push(e) : this.w3_scripts.normal.push(e) : this.w3_scripts.normal.push(e)
		}))
		document.querySelectorAll("script[type=lazyExJs]").forEach((e => {
	...
	preload_scripts(resource) {
		...
		[...resource].forEach((t => {
			...
			if (n) {
				...
				t.href = n, type1 == "module" ? t.rel = "modulepreload" : t.rel = "preload", t.as = "script", e.appendChild(t)
			}else if(j){
				const t = document.createElement("link");
	...
	static execute() {
		...
		if(!e.w3_lazy_load_js || window.scrollY > 50){
...
if(w3_js_is_excluded){
	document.readyState === 'complete' ? setTimeout(function(){w3_loadscripts.execute();},500) : window.addEventListener("load",function(){	setTimeout(function(){w3_loadscripts.execute();},500);});
}else{
